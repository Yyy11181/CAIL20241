            ...
            if filename.endswith('.json'):
                ...
                with open(file_path,'r',encoding='utf-8') as f:
                    ...
                    for item in data_list:
                        factGet = item.get('fact')  #这个是案件的事实部分~
                        id  = item.get('id') #这个是我自己新加的~
                        fact =  f"事实部分：{factGet}"
                        if(field=="judgement"):
                            judgement = item.get('judgement')
                            output = f"判决结果部分：{judgement}"
                            new_item ={"instruction":  "案件编号:"+ id +"," + instruction, "input":fact, "output": output}
                        elif(field=="cause"):
                            ...
                            output = f"判决说理部分：{reasoning}"
                            new_item ={ "instruction": "案件编号:"+ id +"," + instruction, "input":fact, "output": output}
                        elif(field=="ethics_or_jurisprudence"):
                            reasoning=item.get('reasoning')
                            fact = f"判决说理部分：{reasoning}" #这个变为了判决说理部分
                            ethics_or_jurisprudence=item.get('ethics_or_jurisprudence')
                            output = f"伦理或法理部分：{ethics_or_jurisprudence}"
                            new_item ={"instruction":"案件编号:"+ id +"," + instruction, "input":fact, "output": output}
                        # if(flag):
                        #     flag=False
                        #     print(output)
...
def main():
    folder_path = 'cpwsslsc/fu_contest/' #这个是文件夹路径
    output_path='train_ethics_or_jurisprudence.json'
